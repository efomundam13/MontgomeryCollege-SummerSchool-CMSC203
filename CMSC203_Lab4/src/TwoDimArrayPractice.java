 ...
 * Description: Finish writing a couple methods in TwoDomArrayPractice
 * Due: 07/10/2022
...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   ...
   public void setValues( int value, int row ) {
	   System.out.println("Set Value for row " + row + " with value equaling " + value);
	   for(int i = 0; i < intArray.length; i++) {
		   if(i == row) {
			   for(int j = 0; j < intArray[i].length; j++) {
				   intArray[i][j] = value;
...
// Note:  To animate the algorithm, put this method call as the
   ...
   public int findMinimum( int column ) {
	   System.out.println("Find Minimum from Column equals " + column);
	   ...
	   System.out.println("Minimum of in column " + column + " is " + minValue);
   ...
   */
   ...
   public int countFound(int value) {
	   System.out.println("Count Frequency of value equals " + value + " in the matrix.");
